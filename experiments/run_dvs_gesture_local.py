"""
HAMT DVS-Gesture Benchmark (Local GPU)

DVS-Gesture: 11 gesture classes from DVS128 event camera.
This is the most important neuromorphic test because gestures
contain repeated motion patterns that habituation should suppress.

Sensor: 128x128x2 polarities, downsampled to 32x32x2 = 2048 features.
Only 1342 train / 288 test samples, so it runs fast.

Config: Default HAMT (lambda_e=0.001, lambda_h=0.0005, h=800, T=25)
GPU: GTX 1650 (model is ~9MB, fits easily in 4GB VRAM)

Run AFTER run_neuromorphic_local.py completes.
...
def load_dvs_gesture(data_dir):
    """
    DVS-Gesture: 11 gesture classes from DVS128 camera.
    Downsample 128x128 to 32x32, 2 polarities = 2048 input features.
    """
    ...
    print("Loading DVS-Gesture (may download ~1.5GB on first run)...", flush=True)
...
def main():
    ...
    print(f"DVS-Gesture downsampled to {DOWNSAMPLE}x{DOWNSAMPLE}", flush=True)
    ...
    if "dvs_gesture" in all_results:
        print("DVS-Gesture already completed, skipping", flush=True)
        return
    ...
    print(f"\n{'='*70}", flush=True)
    print("DATASET: DVS-Gesture", flush=True)
    print(f"{'='*70}", flush=True)

    train_loader, test_loader, input_size, num_classes = load_dvs_gesture(data_dir)
    ...
    result = {
        ...
        "downsample": f"{DOWNSAMPLE}x{DOWNSAMPLE}",
    ...
    all_results["dvs_gesture"] = result

    with open(results_file, "w") as f:
        json.dump(all_results, f, indent=2, default=str)

    print(f"\nRESULT DVS-Gesture:", flush=True)
